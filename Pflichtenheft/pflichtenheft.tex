...
\makenoidxglossaries
	...
	\newglossaryentry{regex}{
		name={Regulärer Ausdruck},description={\textbf{Reg}ular \textbf{Ex}pressions (RegEx) dienen der Beschreibung von Zeichenketten mit ähnlichem Format. Sie können als Filterkriterien in der Textsuche benutzt werden. Dabei wird der Text mit dem Muster des regulären Ausdrucks verglichen.}
	}
	...
	\newglossaryentry{obs}{
		name={Observation},plural={Observations},description={Eine Observation (deutsch: Beobachtung) ist die Messung eines Wertes einer Eigenschaft}
	}
...
\title{CHILLImport: Weboberfläche zum Management von Sensordaten\\\vspace{2cm} \Large Pflichtenheft\\ \vspace{4cm}}
...
\pagenumbering{arabic}
	...
	Der \textbf{FR}aunhofer\textbf{O}penSource\textbf{S}ensor\textbf{T}hings-Server (\gls{frost})\footnote{https://www.iosb.fraunhofer.de/servlet/is/80113/} des \gls{iosb} ist eine Implementierung des SensorThings-Standards. Der Server will die \gls{stApi} ressourcenschonend und mit hoher Leistungsfähigkeit bereitstellen. Das Institut hat sich direkt von Anfang an dazu entschlossen, die Software erreichbarer zu machen und entwickelte den Server als Open Source Software\footnote{https://github.com/FraunhoferIOSB/FROST-Server}. \\
	...
	\ \\
	Die hier entwickelte Software CHILLImport stellt zu dem \gls{frost} eine Anwendung bereit, die es Nutzern ermöglicht, Daten aus Tabellen (beispielsweise einer Excel-Tabelle) in ein \gls{stApi}-freundliches Format zu konvertieren und auf eine \gls{frost}-Instanz hochzuladen. Die Software soll auf der einen Seite so einfach zu bedienen sein, dass auch technisch unerfahrene Nutzer die Grundfunktionen nutzen können. Auf der anderen Seite soll sie stark konfigurierbar sein, um auch erfahrene Nutzer mit hohen Ansprüchen zufrieden zu stellen.\\
	
	Der Name \string"CHILLImport\string" orientiert sich an dem des \gls{frost}s und spielt ebenso auf den einfachen Import von Sensordaten an.
...
\newpage
	...
	\item Thing (Gegenstand): Ein Objekt aus der realen Welt (physisches Thing) oder ein virtuelles Objekt
	\item Location (Ort): Beschreibt den (zuletzt bekannten) Ort eines oder mehrerer Things
	\item HistoricalLocation (historischer Ort): Beschreibt einen vorherigen Ort des/der Things zusammen mit ihrem Zeitpunkt
	\item Sensor: Misst eine Eigenschaft um eine Approximation des Wertes der Eigenschaft zurückzugeben
	\item Observation (Beobachtung): Die Messung eines Wertes einer Eigenschaft
	\item ObservedProperty (beobachtete Eigenschaft): Spezifiziert das Phänomen einer Beobachtung
	\item FeatureOfInterest (relevantes Phänomen): Eine Beobachtung erzeugt einen Wert, welcher an ein Phänomen gekoppelt ist. Dieses Phänomen ist Teil eines Features, welches das FeatureOfInterest einer Beobachtung ist
...
\section{Zielbestimmung}
	...
	Die Sensor-Messwerte sollen aus Tabellen im \gls{csvExcel}format in ein \gls{frost} Instanz importiert werden.

	\criterium{Konvertierung des Formats der Sensordaten}{crt:conv}
	Die Daten sollen aus den beliebigen Anordnungen innerhalb der Quelldatei (Spaltenformat) in den einheitlichen Standard der \gls{stApi} konvertiert werden.
	...
	Der Benutzer soll Spaltenformat (insbesondere Zuordnung von Spalte zu \gls{ds}), Datenformat, Datums- und Zeitformat einstellen und als Konfiguration abspeichern können.
	...
	Für eine Auswahl gängiger Eingabedateien wird dem Benutzer eine Liste vordefinierter Konfigurationen, bestehend aus Spaltenformat (insbesondere Zuordnung von Spalte zu \gls{ds}), Datenformat und Datums- und Zeitformat, vorgeschlagen.
	...
	\criteriumOptional{Komplexe Transformation/\gls{aggr}}{crt:aggr}
	Dem Benutzer wird eine Möglichkeit gegeben für ausgewählte \gls{ds} eine \gls{aggr} zu definieren. Beim Import neuer Daten wird automatisch sichergestellt, dass die entsprechenden \glspl{aggr} neu berechnet werden.
	...
	Die Sprache der Anwendung ist konfigurierbar. Standardmäßig wird die Spracheinstellung des Browsers übernommen.
...
Jeder kann somit alle gespeicherten Konfigurationen sehen und verwenden.
	\criteriumNot{Keine Kompatibilität mit Tablets und Smartphones}{crt:no-mobile}
	Die Software muss nicht auf Tablets oder Smartphones laufen können.
...
\begin{itemize}
	...
	\begin{itemize}
		...
		\item Ein Bildschirm wird nicht benötigt (\gls{headless})
	\end{itemize}
...
\begin{itemize}
	...
	\item Konnektivität zum Web-Server
\end{itemize}
...
\subsection{Erforderliche funktionale Anforderungen}
	...
	\fulfills{crt:web}
	\fulfills{crt:frost}
	Die Quelle für die Tabellendatei ist frei wählbar aus den folgenden Möglichkeiten:
		\begin{enumerate}
			\item Eine lokale Datei über Dateiupload
			\item Eine entfernte Datei über einen Weblink
			\item Ein anderer \gls{frost}
		\end{enumerate}
		Allerdings dürfen als Datei bei \textit{1.} und \textit{2.} lediglich \gls{csvExcel}en ausgewählt werden.
		
	\functionality{Laden und speichern von Konfigurationen}{fnc:loadsave}
	\fulfills{crt:conf}
	Der Nutzer kann seine Konfiguration auf dem Webserver speichern und diese bei späterer Nutzung wieder laden. Es können mehrere Konfigurationen gespeichert werden, diese werden in einem Drop-Down-Menü angezeigt.
	...
	\begin{itemize}
			...
			\end{itemize}
			\item Erstellen von neuen Entitäten
...
Die Software stellt eine Weboberfläche zum Importieren von Sensor-Datensätzen auf einen \gls{frost} zur Verfügung. 
...
\begin{itemize}
\item Unter "{Configuration}" kann eingestellt werden, in welcher Form die Daten in der Quelldatei abgespeichert sind, um diese dann in den vom \gls{frost} verwendeten \gls{ogc}-Standard zu konvertieren.
...
\item Unter "{DateTime}"{ ist} das Datumsfomat einstellbar. Erst wird die Spalte der Zeitzone eingegeben bzw., falls diese nicht vorhanden ist, über "{Choose TZ}"{ eine} Zeitzone ausgewählt. Danach lassen sich das Format für das Datum bzw. die Zeit als \gls{regex} und die zugehörige Spaltennummer angeben.
...
\caption{\label{fig:oprop}Dialogfenster zum Erstellen einer Observed Property.}
\end{figure}
...
\pagebreak
	...
	\begin{itemize}
		\item Die Software dokumentiert Fehler jeglicher Art und das Starten bzw. Abschließen von Imports auf einen \gls{frost} .
	...
	\teststep{Der Nutzer hat die Quelle eingegeben und bestätigt.}{Der Nutzer füllt die Datums-/Uhrzeit-Spalte aus.}{Die Software überprüft die Korrektheit der Spalten (Wurde eine Spalte angegeben, die nicht existiert?).}	
	\teststep{}{Der Benutzer drückt den "{Add Column}"{-Button}, um eine neue Datums-/Zeit-Spalte anzulegen.}{Die Software erstellt zwei neue Textfelder mit \string"Spalte\string" und \string"Format\string" im Interface, sodass sich die Datums- und Zeitangabe über mehrere Spalten erstrecken kann.}
	...
	\teststep{Ein \gls{thing} wurde ausgewählt.}{Der Anwender wählt den \gls{ds} aus dem \gls{ds}-Dropdown-Menü aus.}{Das System lädt die zum (Multi-)\gls{ds} gehörenden Messeinheiten neben die Textfelder zum eintragen.}
	\teststep{Die Messeinheiten sind geladen.}{Der Nutzer trägt die Spalten mit den Messwerten neben die Messeinheiten ein.}{Die Korrektheit der Eintragung wird wieder durch die Software geprüft.}
	\teststep{Alle Textfelder sind ausgefüllt.}{Der User klickt auf den Button "{Import Data}".}{Die Daten in den angegebenen Spalten werden eingelesen, konvertiert und auf den \gls{frost} übertragen.}
	
	\test{Import fehlerhafter Daten}{tst:fehler}
	\tests{fnc:fehler}
	\teststep{Die Konfiguration ist vollständig ausgefüllt, aber die Quelldatei enthält fehlerhafte Daten.}{Der User klickt auf den Button \string" Import Data\string".}{Die Software verarbeitet alle fehlerfreien Datenzeilen und überspringt fehlerhafte Zeilen. Anschließend wird im Log vermerkt, welche Zeilen ausgelassen wurden.}
	...
	\tests{fnc:loadsave}
	\teststep{Der Nutzer hat eine Konfiguration erstellt.}{Der Nutzer drückt den "{Save Configuration}"{-Knopf}.}{Es wird eine cfg-Datei der Konfiguration erstellt und abgespeichert.}
...
%	\ \\
	...
	\tests{fnc:default}
	\teststep{Der Nutzer befindet sich auf der Webseite und es wird der Konfigurator angezeigt.}{Der Nutzer drückt den "{Reset Configuration}"{-Knopf}.}{Alle eingestellten Konfigurationen werden auf den Default-Case zurückgesetzt.}
	
	
		
	\test{Neues Thing erstellen}{tst:thing}
	\tests{fnc:create}
	\teststep{Die Datei und die \gls{frost}-Domain wurden angegeben/hochgeladen.}{Der Nutzer klickt auf den "{Create Thing}"{-Button}.}{Es öffnet sich der "{Create New Thing}"{-Dialog} (\cref{fig:thing}).}
...
\pagebreak
	...
	\test{ObservedProperty erstellen}{tst:oprop}
	\tests{fnc:create}
	...
	\tests{fnc:help}
	\teststep{Der Nutzer befindet sich auf der Webseite.}{Der Nutzer drückt den "{Help}"{-Button}.}{Es wird eine Anleitung für den Gebrauch der Weboberfläche angezeigt.}
