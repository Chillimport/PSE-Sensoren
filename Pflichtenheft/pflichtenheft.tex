...
Der FROST-Server vom Fraunhofer IOSB ist eine Open-Source-Implementierung des SensorThings-Standards des OGC und unter https://github.com/FraunhoferIOSB/FROST-Server downloadbar. Der FROST-Server dient der Speicherung und Verwaltung von Sensordaten.\\
...
\item OGC \\
	Das Open Geospatial Consortium (OGC) ist eine gemeinnützige Organisation, die sich zum Ziel gesetzt hat, die Entwicklung von raumbezogener Informationsverarbeitung (insbesondere Geodaten) auf Basis allgemeingültiger Standards festzulegen.
\item SensorThings API \\
...
\item GUI \\
	Ein Grafische Benutzeroberfläche (engl. graphical user interface, GUI) hat die Aufgabe, eine Anwendungssoftware auf einem Rechner mittels grafischer Symbole und Steuerelemente für den Benutzer bedienbar zu machen.
\item (Java) JRE \\
	Mit der Java-Laufzeitumgebung (Java Runtime Environment, JRE) werden Programme (Java-Anwendungen) weitgehend unabhängig vom darunter liegenden Betriebssystem ausgeführt.
\item JavaScript \\
...
\item Regex \\
	Regular Expressions (Regex, regulärer Ausdrucke) dienen der Beschreibung von Zeichenketten mit ähnlichem Format.
...
\item HTTPS/SSL \\
	Hypertext Transfer Protocol Secure (HTTPS) ist ein sicheres Hypertext-Übertragungsprotokoll, das SSL benutzt.
...
\item GPLv3
	GPLv3 ist die aktuelle Versioon von GPL. Diese General Public License (allgemeine Veröffentlichungsgenehmigung) ist die am weitesten verbreitete Softwarelizenz,
